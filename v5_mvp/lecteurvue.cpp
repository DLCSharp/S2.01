...
void LecteurVue::majInterface(Lecteur::modesLecteur m, const QMap<QString, QString> &mapLabels)
{
    switch (m) {
    ...
    case Lecteur::MANUEL :
        majInterface_lecteurManuel(mapLabels);
        break;

    case Lecteur::AUTOMATIQUE :
        majInterface_lecteurAutomatique(mapLabels);
        break;
...
void LecteurVue::majInterface_lecteurVide()
{
    ...
    ui->labelDiapoCourant->setText("Pas de diaporama actuellement chargé. Chargez un diaporama depuis le menu 'Fichier' !");
    ui->groupBoxImages->setTitle("Pas d'image à afficher");
    ui->labelImage->clear();
...
void LecteurVue::majInterface_lecteurManuel(const QMap<QString, QString> &mapLabels)
{
    ...
    ui->labelDiapoCourant->setText("Diaporama courant : " + mapLabels.value("intituleDiapoCourant"));
    ui->groupBoxImages->setTitle("Image " + mapLabels.value("numImageCourante") + " sur " + mapLabels.value("nbImages"));
    ui->labelImage->setPixmap(QPixmap(mapLabels.value("cheminImage")));
...
void LecteurVue::majInterface_lecteurAutomatique(const QMap<QString, QString> &mapLabels)
{
    ui->btnReculer->setEnabled(true);
    ui->btnAvancer->setEnabled(true);
    ui->btnLancer->setEnabled(true);
    ...
    labelVitesse->setText("Vitesse : " + mapLabels.value("vitesseDefil"));
}
...
void LecteurVue::paramVitPerso_declenche()
{
    ...
    bool diagVitesseValide = false;

    unsigned int vitesse = QInputDialog::getInt(this, tr("Définition d'une vitesse de défilement personnalisée"),
                                                tr("Entrer une vitesse de défilement personnalisée :"), 1,
                                                1, 2147483647, 1, &diagVitesseValide);

    if (diagVitesseValide)
    {
        qDebug("[Vue] Envoi de la demande de definition de vitesse de defilement a : %d", vitesse);

        emit signalDemandeDefVitesse(vitesse);
    }
    else
    {
        qDebug("[Vue] Annulation de la definition de vitesse de defilement personnalise");
    }
}
